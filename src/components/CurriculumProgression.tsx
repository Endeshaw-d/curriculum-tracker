...
<div className="flex justify-end text-sm text-gray-700 pr-4 pt-2">
  {(() => {
    const allTopics = curriculumData[subject].flatMap((y) => y.topics);
    const completed = allTopics.filter((t) => progress[t.code]).length;
    const total = allTopics.length;
    const percent = Math.round((completed / total) * 100);
    const remaining = total - completed;
    return `${percent}% total completed — ${remaining} topics remaining`;
  })()}
</div>
              ...
              {userProgress.map(({ name, percent }) => (
                ...
                >
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm text-gray-600">{percent}% completed<br/><span className="text-xs">{timestamp ? new Date(timestamp).toLocaleString() : 'No activity yet'}</span></span>
                  </div>
